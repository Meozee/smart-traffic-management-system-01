...
_DENSITY_LOW_THRESHOLD_PERCENT: float = float(os.getenv("DENSITY_LOW_THRESHOLD", "40"))
_DENSITY_HIGH_THRESHOLD_PERCENT: float = float(os.getenv("DENSITY_HIGH_THRESHOLD", "70"))

DENSITY_LOW_THRESHOLD: float = _DENSITY_LOW_THRESHOLD_PERCENT / 100.0
"""
Threshold untuk density level 'Low' (dalam ratio 0.0-1.0).
Low: < 0.40, Medium: 0.40-0.70, High: > 0.70
"""

DENSITY_HIGH_THRESHOLD: float = _DENSITY_HIGH_THRESHOLD_PERCENT / 100.0
"""
Threshold untuk density level 'High' (dalam ratio 0.0-1.0).
Density level jadi 'High' jika ratio > threshold ini.
"""
...
Setiap interval, sistem akan:
  1. Query deteksi dalam interval waktu terakhir
  2. Agregasi per camera
  3. Hitung density ratio & level
  4. Simpan ke tabel TRAFFIC_DENSITY
  5. Jika level = High, buat ALERT
"""
...
def validate_config() -> None:
    ...
    if not (0.0 <= DENSITY_LOW_THRESHOLD <= 1.0):
        errors.append(
            f"❌ DENSITY_LOW_THRESHOLD harus 0.0-1.0, ditemukan: {DENSITY_LOW_THRESHOLD}"
        )

    if not (0.0 <= DENSITY_HIGH_THRESHOLD <= 1.0):
        errors.append(
            f"❌ DENSITY_HIGH_THRESHOLD harus 0.0-1.0, ditemukan: {DENSITY_HIGH_THRESHOLD}"
        )

    if DENSITY_LOW_THRESHOLD >= DENSITY_HIGH_THRESHOLD:
        errors.append(
            f"❌ DENSITY_LOW_THRESHOLD ({DENSITY_LOW_THRESHOLD}) harus < "
            f"DENSITY_HIGH_THRESHOLD ({DENSITY_HIGH_THRESHOLD})"
        )
...
def print_config_summary() -> None:
    ...
    print(f"✓ DENSITY_LOW_THRESHOLD: {DENSITY_LOW_THRESHOLD:.2%}")
    print(f"✓ DENSITY_HIGH_THRESHOLD: {DENSITY_HIGH_THRESHOLD:.2%}")
