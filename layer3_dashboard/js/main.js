...
//   3. [FIX] Menggunakan apiFetch() untuk keamanan JWT.
//   4. [FIX] Menghapus Ghost URL dan sinkronisasi dengan endpoint /api/v1/alerts
// ─────────────────────────────────────────────
...
async function loadPage(evt, pageName) {
    ...
    if (!pageContainer) {
        ...
        try {
            // Ini fetch file statis HTML, jadi aman pakai fetch biasa
...
async function loadSidebarCameras() {
    ...
    try {
        // FIX: Menggunakan apiFetch dari api.js agar JWT token terkirim
        const res = await apiFetch(buildApiUrl('/api/v1/cameras/'));
...
setInterval(async () => {
    ...
    try {
        // FIX: Rute baru ke /alerts/ (otomatis mengambil yang statusnya active dan terbaru)
        const res = await apiFetch(buildApiUrl('/api/v1/alerts/'));
        if (!res || !res.ok) return;
        ...
        if (latest && latest.alert_id > lastAlertId) {
            ...
            try {
                // FIX: Rute baru untuk acknowledge menggunakan POST
                await apiFetch(buildApiUrl(`/api/v1/alerts/${latest.alert_id}/acknowledge`), { method: 'POST' });
            } catch (e) { console.warn('Gagal mark alert acknowledge', e); }
...
document.addEventListener("DOMContentLoaded", () => {
    if (checkAuth()) {
        ...
        loadSidebarCameras();
        setInterval(loadSidebarCameras, 30000); // Sidebar refresh setiap 30 detik
    }
});